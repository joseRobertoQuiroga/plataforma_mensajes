...
> **Archivos totales:** ~58 documentos | **Líneas totales:** ~16,000 | **Última actualización:** Julio 2026
> **📖 Hub Visual:** Abre `hub/index.html` en navegador para versión interactiva.
> **📌 Documentos maestros:** `ROADMAP-MULTI-AGENT-MEMORY-CONTEXT.md` · `SECURITY-MASTER.md` · `UI-UX-MASTER.md`
...
|---------|--------|----------------|-------------------|
| `ESTADO-GENERAL.md` | 129 | Barra de progreso por área, tabla de componentes con estado, top 5 próximos pasos | Vista rápida del estado general del proyecto |
| `COMPONENTES.md` | 115 | Matriz de salud de cada servicio (puerto, estado, health check, versión, DB) | Diagnosticar qué servicio está funcionando o no |
| `LOGROS.md` | 136 | Todos los logros completados organizados por dominio (infra, helper, Dify, n8n, Twenty, docs) | Saber qué se ha hecho hasta ahora |
...
| `PROCEDIMIENTOS.md` | 190 | Comandos: start/stop, acceso a servicios, post-reset, Meta WhatsApp, health verification, backup | Operaciones del día a día |
...
| Archivo | Líneas | ¿Qué contiene? | ¿Para qué sirve? |
|---------|--------|----------------|-------------------|
| `ARCHITECTURE.md` | 102 | Diagrama de arquitectura general, stack de 11 servicios, mapa de puertos, flujos inbound y broadcast | Entender cómo se conectan los servicios |
...
### 📁 `scripts/` — Automatización y Base de Datos (4 scripts + SQL)
...
| Archivo | Líneas | ¿Qué contiene? | ¿Para qué sirve? |
|---------|--------|----------------|-------------------|
| `index.js` | 1,672 | Servidor Express: 35+ endpoints (campañas, leads, scoring, templates, Twenty sync, Excel upload, LLM chat, webhooks) | Lógica de integración central |
...
| Archivo | Líneas | ¿Qué contiene? | ¿Para qué sirve? |
|---------|--------|----------------|-------------------|
| `docker-compose.yml` | 481 | Orquestación de 11+ servicios con health checks, redes, volúmenes | Levantar todo el sistema |
| `nginx.conf` | 254 | Proxy reverso: rutas /hub/, /chatwoot/, /dify/, /n8n/, /crm/, /admin/, /api/, /webhooks/ | Unificar acceso a todos los servicios |
| `.env` | 46 | Variables de entorno activas (DB, API keys, secretos) | Configurar servicios |
| `.env.example` | 54 | Template de variables con placeholders | Guía para generar .env |
...
> **Propósito:** Planificación a futuro de grandes módulos del sistema.
...
| `SECURITY-MASTER.md` | 1,082 | Análisis de vulnerabilidades: 7 críticas, 12 altas, 15 medias, 9 bajas. Superficie de ataque por componente, mitigaciones, roadmap de hardening en 5 fases | Auditoría y hardening de seguridad |
...
📖 DOCUMENTACIÓN       → docs/ (17 archivos)
...
🆕 PLANES MAESTROS     → raíz (6 MD: roadmap, security, ux, ops, data, business)
🔗 CRUCES Y FASES      → raíz (2 MD: fase1-mvp-cruzado, fases-cruzadas)
```
