...
describe('Flujos de Negocio (Business Flows) — pipeline nativo Wibsite 2.0', () => {
  ...
  afterEach(() => {
    ...
    storeFacade.getStore().businessHours = { start: 0, end: 24, timezone: 'UTC' };
  ...
  // C1: el motor de campa�as debe excluir opt-outs en todo env�o
  ...
  // C6: campa�as por evento de fecha (aniversario/cumplea�os)
  test('C6: trigger de fecha dispara la campa�a para leads del d�a', async () => {
    const store = storeFacade.getStore();
    const today = new Date().toISOString().slice(0, 10);
    store.leads.push({ id: 'C6-bday-1', name: 'Cumplea�ero', phone: '+591C6000001', email: 'bday1@example.com', status: 'active', custom_fields: { birthday: today } });
    ...
    const resCamp = await request(app)
      ...
      .send({
        ...
        message_template: 'Hola {{name}}, feliz cumplea�os ??',
        audience_filter: { all: true },
  ...
  test('C6: campa�a por evento excluye leads opted_out', async () => {
    const store = storeFacade.getStore();
    const today = new Date().toISOString().slice(0, 10);
  ...
  // C4: campa�as de reactivaci�n autom�tica
  test('C4: audiencia de reactivaci�n coincide con la regla (score>=40 + sin reply en 14d)', async () => {
    const store = storeFacade.getStore();
  ...
  test('C4: campa�a de reactivaci�n dispara sobre la audiencia', async () => {
    const oldDate = new Date(Date.now() - 20 * 86400000).toISOString();
    storeFacade.updateStore(s => {
      s.leads.push(
    ...
    const resCamp = await request(app)
      ...
      .send({
        ...
        message_template: 'Hola {{name}}, �segu�s interesado?',
  ...
  test('C5: split 50/50 determin�stico asigna variantes consistentes', async () => {
    const store = storeFacade.getStore();
    const variants = [
    ...
    expect(firstPass).toEqual(secondPass); // determin�stico
  ...
  test('C5: ab-report separa m�tricas por variante y declara ganadora', async () => {
    const store = storeFacade.getStore();
    const resCamp = await request(app)
      .post('/api/campaigns')
      .set('x-api-key', API_KEY)
  ...
  // C10: atribuci�n MVP campa�a ? replies ? avance de etapa
  test('C10: atribuci�n correcta sobre dataset sint�tico', async () => {
    const store = storeFacade.getStore();
    const resCamp = await request(app)
      .post('/api/campaigns')
      .set('x-api-key', API_KEY)
    ...
    store.leads.push(
      { id: 'C10-l1', name: 'Avanz�', phone: '+591C1000001', email: 'c10a@example.com', status: 'interesado', score: 70, entry_stage: 'primer_contacto' },
  ...
  // C12: validaci�n de plantillas por canal (HSM vs sesi�n)
  test('C12: plantilla HSM marcada rechaza env�o directo fuera de sesi�n (simulado)', async () => {
    ...
    // Validaci�n por canal: WhatsApp + HSM sin aprobaci�n ? inv�lido
    ...
    // Mismo template en Telegram (sesi�n libre) ? v�lido
  ...
  test('R12: comando MENU responde con men� principal sin llamar LLM', async () => {
    const res = await request(app)
      .post('/webhooks/twilio-inbound')
      .type('form')
    ...
    expect(res.body.response).toContain('principa');
