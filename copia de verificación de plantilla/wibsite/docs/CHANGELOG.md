...
### Known Issues (v2.1.1)
- Meta/WhatsApp: credenciales pendientes (META_APP_ID, WHATSAPP_PHONE_NUMBER_ID, etc.)
- Chatwoot webhook: requiere credenciales Meta para completar registro
- Workflow Dify lead classifier: pendiente de importar y migrar a OpenRouter
...
- **Score history no se registra en evaluate-all** — solo lead.score + lead.score_data se actualizan, sin entrada en s.scores.
- **Preview de template no valida que todas las variables existan** — variables faltantes se renderizan como `{{varName}}` sin warning.
- Dify OpenRouter config: requiere ejecutar `scripts/configure-openrouter.js` después de levantar Docker
- n8n: deprecation warning `N8N_SKIP_WEBHOOK_DEREGISTRATION_SHUTDOWN` — remover variable de entorno.
